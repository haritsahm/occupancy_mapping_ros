  ...
  :nh_(nh)
{
  ...
  scanner.linear() = Eigen::Quaterniond(0,0,0,1).toRotationMatrix();
  scanner.translation() = Eigen::Vector3d(-0.032, 0.000, 0.172);
...
double OccupancyMap::updateCell(bool occ)
{
  double x = 0.3;
  if(occ)
    x = 0.6;
...
void OccupancyMap::updateMap()
{
  ...
  for(int index=0; index < laser_data.ranges.size(); index++)
  {
    ...
    Point2D r_pos_w_map(map_center.x + laser_pos.x(), map_center.y-laser_pos.y());
    ...
    if(hit)
    {
      m_hit = map_point[map_point.size()];
      map_point.pop_back();
    }

    for(auto &p: map_point)
    {
      int px = (int)round(p.x); int py = (int)round(p.y);
      map(py,px)=logs_map(py,px) + loggOddRatio(false) - lo;
      logs_map(py,px) = map(py,px);
    }

    int px = (int)round(m_hit.x); int py = (int)round(m_hit.y);
    map(py,px) = logs_map(py,px) + loggOddRatio(true) - lo;
    logs_map(py,px) = map(py,px);
