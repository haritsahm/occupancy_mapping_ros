...
void OccupancyMap::updateMap()
{
    ...
    Eigen::Vector3d laser_pos_m = laser_pos*10; laser_pos_m.array().round();
    ...
    for(int index=0; index < laser_data.ranges.size(); index++)
    {
        ...
        for(auto &p: map_point)
        {
            ...
            map(py,px)=clip(map(py,px) + loggOddRatio(false) - lo);
        }

        if(hit)
        {
            int px = (int)floor(m_hit.x); int py = (int)floor(m_hit.y);
            map(py,px) = clip(map(py,px) + loggOddRatio(true) - lo);
        }
    }
